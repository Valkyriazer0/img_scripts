...
def main():
    ...
    image = cv2.imread(r"C:\Users\zer0\Downloads\reference\mitte20510004.jpg", cv2.IMREAD_COLOR) / 255.0
    image = img_module.img_transform(image, scale=0.1)

    scribbles = cv2.imread(r"C:\Users\zer0\Downloads\edge_ratio.png", cv2.IMREAD_COLOR) / 255.0
    # scribbles = img_module.img_transform(scribbles, scale=0.1)
    # alpha = closed_form_matting_with_scribbles(image, scribbles)

    trimap = cv2.imread(r"C:\Users\zer0\Downloads\edge_ratio.png", cv2.IMREAD_GRAYSCALE) / 255.0
    # trimap = img_module.img_transform(trimap, scale=0.1)
    alpha = closed_form_matting_with_trimap(image, trimap)

    cv2.imwrite(r"C:\Users\zer0\Downloads\output_alpha.png", alpha * 255.0)
